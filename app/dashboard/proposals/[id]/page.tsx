    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-3">
          <Button asChild variant="outline" className="rounded-xl border-white/10 bg-white/[0.04]">
        ...
        <header className="rounded-2xl border border-white/10 bg-white/[0.035] p-5 backdrop-blur-xl">
          <p className="text-sm text-blue-300">Proposal details</p>
        ...
        <section className="grid gap-4 lg:grid-cols-[1.4fr_0.8fr]">
          <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-5 backdrop-blur-xl">
            ...
            <p className="whitespace-pre-wrap text-sm leading-6 text-zinc-300">{proposal.coverLetter}</p>
          </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-5 backdrop-blur-xl">
              ...
              <dl className="grid gap-3 text-sm">
                <div className="flex justify-between gap-3">
                  ...
                  <dd className="text-zinc-200">{proposal.job?.company ?? "External client"}</dd>
                </div>
                <div className="flex justify-between gap-3">
                  <dt className="text-zinc-500">Budget</dt>
                  <dd className="text-zinc-200">{proposal.budget ? `$${proposal.budget.toLocaleString()}` : "Open"}</dd>
                </div>
                <div className="flex justify-between gap-3">
                  <dt className="text-zinc-500">Timeline</dt>
                  <dd className="text-zinc-200">{proposal.timeline ?? "Flexible"}</dd>
                </div>
              </dl>
            </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-5 backdrop-blur-xl">
              ...
              <div className="grid gap-3">
                {timeline.map(([label, date]) => (
                  <div key={label} className="flex items-center justify-between gap-3 rounded-xl border border-white/10 bg-zinc-950/70 px-3 py-2 text-sm">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-5 backdrop-blur-xl">
            ...
            <div className="grid gap-2">
              {proposal.attachments.map((attachment) => (
                <a key={attachment.id} href={attachment.fileUrl} target="_blank" rel="noreferrer" className="rounded-xl border border-white/10 bg-zinc-950/70 px-3 py-2 text-sm text-blue-200 hover:bg-white/[0.05]">
                  {attachment.fileName}
                </a>
              ))}
            ...
            {proposal.job?.skills?.length ? (
              <div className="mt-4 flex flex-wrap gap-2">
                {proposal.job.skills.map((skill) => (
                  <span key={skill} className="rounded-full border border-white/10 bg-white/[0.04] px-2 py-1 text-xs text-zinc-300">
                    {skill}
                  </span>
                ))}
              </div>
