    ...
    <motion.section initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.45 }}>
      <Card className="rounded-2xl border-white/10 bg-white/[0.045] shadow-2xl shadow-black/20 backdrop-blur-xl">
        <CardHeader className="flex-row items-center justify-between gap-3">
          <CardTitle className="text-base text-zinc-100">Compare plans</CardTitle>
          ...
          <Table>
            ...
            <TableBody>
              {rows.map(([category, free, pro, enterprise]) => (
                <TableRow key={category} className="border-white/10 hover:bg-white/[0.03]">
                  <TableCell className="font-medium text-zinc-100">{category}</TableCell>
                  <TableCell className="text-zinc-400"><CellValue value={free} /></TableCell>
                  <TableCell className="text-blue-100"><CellValue value={pro} /></TableCell>
                  <TableCell className="text-zinc-300"><CellValue value={enterprise} /></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
