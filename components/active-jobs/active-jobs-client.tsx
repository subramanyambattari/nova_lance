        ...
        <header className="rounded-2xl border border-white/10 bg-white/[0.04] p-6 shadow-2xl shadow-black/20 backdrop-blur-xl">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h1 className="mt-3 text-3xl font-semibold text-white sm:text-4xl">Active Jobs</h1>
...
function StatsGrid({ data }: { data: ActiveJobsResponse }) {
  ...
  return (
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
      {stats.map((stat) => (
        <Card key={stat.label} className="border-white/10 bg-white/[0.045] shadow-xl shadow-black/10 backdrop-blur-xl">
      ...
      <CardContent className="grid gap-3 p-4 lg:grid-cols-[1.2fr_0.75fr_0.75fr_0.75fr]">
        <div className="relative">
          ...
          <Input
            ...
            className="border-white/10 bg-zinc-950/60 pl-9 text-zinc-100"
          />
        </div>
        <Tabs
        ...
        >
          <TabsList className="flex h-auto min-w-full w-max justify-start gap-1 bg-zinc-950/60">
            {statuses.map((status) => (
        ...
        <Select value={filters.client} onChange={(event) => onChange({ ...filters, client: event.target.value })} className="bg-zinc-950/60">
        ...
        <Select value={filters.priority} onChange={(event) => onChange({ ...filters, priority: event.target.value })} className="bg-zinc-950/60">
        ...
        <Select value={filters.sort} onChange={(event) => onChange({ ...filters, sort: event.target.value })} className="bg-zinc-950/60">
...
function JobCard({ job }: { job: ActiveJobItem }) {
  ...
  return (
    <Link href={`/active-jobs/${job.id}`} className="block">
      <Card className="border-white/10 bg-white/[0.045] shadow-2xl shadow-black/10 backdrop-blur-xl transition hover:border-blue-400/40 hover:bg-white/[0.065]">
          ...
          <div>
            ...
            {nextMilestone ? (
              <p className="mt-4 text-sm text-zinc-300">
                Current milestone: <span className="text-white">{nextMilestone.title}</span>
              </p>
            ) : null}
          </div>
          <div className="flex flex-col justify-between gap-4 rounded-xl border border-white/10 bg-zinc-950/50 p-4">
            <div>
            ...
            <div>
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium text-white">{job.paymentProgress}%</span>
              </div>
              <Progress value={job.paymentProgress} className="mt-3 h-2" />
            </div>
            <div className="flex items-center justify-between border-t border-white/10 pt-4">
              <span className="text-sm text-zinc-500">Budget</span>
              <span className="text-lg font-semibold text-white">{currency(job.budget ?? 0)}</span>
            </div>
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <div className="grid gap-4">
      <Card className="border-white/10 bg-white/[0.045] backdrop-blur-xl">
        <CardHeader>
          <CardTitle className="text-base text-white">Live analytics</CardTitle>
        </CardHeader>
          ...
          <div className="h-52 min-h-0 min-w-0">
            <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
              <AreaChart data={data.analytics.weeklyProductivity}>
                ...
                <Tooltip contentStyle={{ background: "#09090b", border: "1px solid rgba(255,255,255,0.12)" }} />
  ...
  return (
    <Card className="border-white/10 bg-white/[0.045] backdrop-blur-xl">
      <CardContent className="p-8 text-center">
        ...
        <h2 className="mt-4 text-lg font-semibold text-white">No active jobs yet</h2>
        <p className="mt-2 text-sm text-zinc-400">Accepted proposals with a client-backed job will appear here automatically.</p>
      </CardContent>
    </Card>
  )
}
      ...
      <div className="mx-auto grid max-w-7xl gap-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
          {Array.from({ length: 5 }).map((_, index) => (
            <Skeleton key={index} className="h-28 bg-white/[0.06]" />
          ))}
        </div>
